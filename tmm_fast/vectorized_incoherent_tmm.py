...
from tmm_fast.vectorized_tmm_dispersive_multistack import (
    SnellLaw_vectorized,
    interface_r_vec,
    interface_t_vec,
    T_from_t_vec,
...
# 该函数通过并行计算处理 incoherent（非相干）和 coherent（相干）光谱在多层薄膜中的传播。利用 PyTorch 的 GPU 加速功能，能够高效地进行计算并支持自动求导。
def inc_vec_tmm_disp_lstack(
    ...
    D : torch.Tensor
        ...
        [n_stacks, n_layers]
        表示所有 incoherent 和 coherent 层的厚度
    mask : list
