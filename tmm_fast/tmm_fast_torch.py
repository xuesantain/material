...
# list_snell 函数通过应用斯涅尔定律计算了光在多层介质中的传播角度，
# 考虑了不同层的折射率和可能的复杂情况，同时确保了传播方向的合理性
...
def list_snell(n_list, th_0):
    ...
    ----------
    n_list : torch tensor
        refractive indices of all layers  各个层的折射率
    th_0 : float
...
# 计算多层薄膜的相干反射和透射光谱，适用于处理具有色散特性的材料，并使用 PyTorch 进行高效计算。
def coh_tmm_fast_disp(pol, n_list, d_list, theta_incidence, lambda_vacuum):
    ...
    res : dict
        'R' : torch tensor 
            ...
            计算的多层薄膜的反射率，单位为绝对反射功率
        'T' :  torch tensor - TODO
            Reflectivity of the computed multilayer thin-film over all concidered 
            wavelengths and angles of incidence
            unit absolut transmitted power, 
            shape [<number of concidered angles of incidence>, <number of concidered wavelength points>]
            计算的多层薄膜的透射率，单位为绝对透射功率
        'r' : torch tensor
...
# 用于计算多层薄膜的相干反射和透射光谱，只处理具有常量折射率的材料
