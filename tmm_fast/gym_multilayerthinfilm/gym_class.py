...
class MultiLayerThinFilm(gymnasium.Env):
    def __init__(self, 
        ...
        """
        初始化多层薄膜（MLTF）优化的新环境。
        每一层是由它的（色散）折射率和厚度决定的。
        因此，除了选择下一层的材料外，强化学习代理还必须为这一层分配厚度。这个公式允许将层的后续堆叠解释为参数化的马尔可夫决策过程。
        ...
        print("self.observation_space is {}",self.observation_space)
        if weights is None:
            self.weights = np.ones_like(self.target)
        else:
    ...
    def step(self, action):
        ...
        # print("---------------------------------")
        # print("observation self.n is",self.n)
        # print("---------------------------------")
        # print("observation self.d is",self.d)
        # print("---------------------------------")
        # print("observation one_hot_status is",one_hot_status)
        # print("---------------------------------")
        # print("shape is ",[len(self.simulation), len(self.n), len(self.d), one_hot_status.shape])
        # print("---------------------------------")
    ...
    def simulate(self, n, d):
        ...
        该方法实现了对特定堆栈反射率的模拟。TMM及其并行化是基于Alexander Luce之前的工作。
                Args:
    ...
    def stack_layers(self, d_array=None, n_array=None):
        ...
        该方法使用预定义包层对代理建议的堆栈进行包层。返回的数组n， d描述了一个特定的堆栈，它包括包层。
            """
